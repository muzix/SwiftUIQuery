...
struct QueryClientTests {
    ...
    func getQueryState() {
        ...
        // Initially no state
        let initialState: QueryState<TestUser, QueryError>? = client.getQueryState(queryKey: queryKey)
        #expect(initialState == nil)
        
        // Set data and check state
        client.setQueryData(queryKey: queryKey, data: testUser)
        let state: QueryState<TestUser, QueryError>? = client.getQueryState(queryKey: queryKey)
    ...
    func buildQueryCreatesNew() {
        ...
        #expect(query.state.status == .pending)
    }
